...
class FeatureExtraction(object):
    def __init__(self):
        self.mdl_proto_downloader = ModelAndPrototxtDownloader()

    def extract_features(self, pretrained_model , foldername_models):
        ...
        takeaction.get(pretrained_model , errhandler)(pretrained_model , foldername_models)


    def _features_alexnet(self , pretrained_model , foldername):
        ...
        model_def = os.path.join(foldername , "bvlc_alexnet_deploy.prototxt")
        pretrained_model = os.path.join(foldername , "bvlc_alexnet.caffemodel")
        batch_size = 10
    ...
    def _features_googlenet(self , pretrained_model , foldername):
        print("\n ----------Checking if appropriate pre trained model is downloaded and also the associated prototxt file --------")
        self.mdl_proto_downloader.download_model_and_prototxt(pretrained_model)

        extract_from_layer = "pool5/7x7_s1"
        input_exp_file = "images.txt"
        model_def = os.path.join(foldername , "bvlc_googlenet_deploy.prototxt")
        pretrained_model = os.path.join(foldername , "bvlc_googlenet.caffemodel")
        batch_size = 10
        self._extract_features(pretrained_model, model_def , extract_from_layer , input_exp_file , batch_size)

#TODOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    def _features_reference_caffenet(self , pretrained_model , foldername):
        print("\n ----------Checking if appropriate pre trained model is downloaded and also the associated prototxt file --------")
        self.mdl_proto_downloader.download_model_and_prototxt(pretrained_model)
...
#TODOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    def _features_finetune_flickr_style(self , pretrained_model , foldername):
        print("\n ----------Checking if appropriate pre trained model is downloaded and also the associated prototxt file --------")
        self.mdl_proto_downloader.download_model_and_prototxt(pretrained_model)
    ...
    def _save_features_to_file(self , features , associated_filenames , modelname , layername):    
        for index , value in enumerate(features):
            ...
            features_folder_path = os.path.join(cwd , "features_test" , modelname , layername )
